...
class CocktailControllerTest extends TestCase
{
    ...
    */
    public function getCocktailsStartingWith_variousLetters_arrayOfCocktails($letter, $expectedCocktails, $data){
        ...
        $actualCocktails = $controllerStub->getCocktailsStartingWith($letter);
        $this->assertEquals($expectedCocktails, $actualCocktails);
    }

    /**
     * @test
     */
    public function getCocktailsStartingWith_stringLetter_checkReadAllIsCalled(){
        $cocktailMock = $this->createMock(Cocktail::class);
        $cocktailMock->method('readAll')->willReturn([['name' => 'Mango Juice', 'description' => 'A description', 'recipe' => 'A recipe']]);
        $controllerStub = $this->getMockBuilder(CocktailController::class)
            ->disableOriginalConstructor()
            ->setMethods(null)
            ->getMock();
        $controllerStub->cocktail = $cocktailMock;

        $cocktailMock->expects($this->once())->method('readAll');
        $controllerStub->getCocktailsStartingWith('m');
    }
